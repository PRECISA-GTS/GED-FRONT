...
const VerticalNavGroup = props => {
    ...
    const toggleActiveGroup = (item, parent) => {
        ...
        if (openGroup.includes(item.title)) {
            ...
            // ** Scroll to bottom when group is clicked
            scrollBottom()
        }
    ...
    const scrollBottom = () => {
        ...
        const duration = 800; // Tempo em milissegundos
        const startTime = performance.now();
        const animateScroll = (timestamp) => {
            const elapsed = timestamp - startTime;
            const progress = Math.min(elapsed / duration, 1); // Garante que o progresso não ultrapasse 1
            div.scrollTop = progress * scrollHeight;
            if (elapsed < duration) {
