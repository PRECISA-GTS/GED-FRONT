...
const StatusFornecedor = ({ statusID }) => {
    ...
    const Steps = ({ total, color }) => {
        return (
            <>
                ...
                <Step color={color} filled={total > 1 ? true : false} title='Acessou formulário' />
                <Step color={color} filled={total > 2 ? true : false} title='Em preenchimento' />
                <Step color={color} filled={total > 3 ? true : false} title='Concluiu preenchimento' />
                <Step
                    color={color}
                    filled={total > 4 ? true : false}
                    title={
                        statusID === 50
                            ? 'Reprovado'
                            : statusID === 60
        ...
        return (
            <div className='grid grid-cols-5 items-center gap-1'>
                {statusID === 10 ? (
                    ...
                    <Steps total={5} color={hexToRGBA(theme.palette.error.main, 1)} />
                ) : statusID === 60 ? (
                    <Steps total={5} color={hexToRGBA(theme.palette.warning.main, 1)} />
                ) : statusID === 70 ? (
                    <Steps total={5} color={hexToRGBA(theme.palette.primary.main, 1)} />
                ) : (
                    <p>--</p>
                )}
            </div>
...
export default StatusFornecedor
