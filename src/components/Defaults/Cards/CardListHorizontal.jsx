...
const CardListHorizontal = ({ xs, md, icon, id, title, subtitle, action, handleClick, linkedsForms }) => {
    ...
    return (
        <Grid item xs={xs} md={md}>
            ...
            >
                ...
                >
                    <Box display='flex' flexDirection='column' alignItems='center' sx={{ gap: 1, padding: 2 }}>
                        <Icon icon={icon} width={38} className='text-[#F8D552]' />
                        <Typography variant='h6' className={`!font-extrabold`}>
                            {title}
                        </Typography>
                        <Typography variant='subtitle2' className='opacity-50'>
                            {subtitle}
                        </Typography>
