...
const AuthProvider = ({ children }) => {
    ...
    const hasSectorPermission = departamentos => {
        if (!departamentos || departamentos.length == 0 || user.admin === 1 || user.papelID == 2) return true
        const hasPermission = departamentos.some(row => user.departamentos.some(userDepartamento => userDepartamento.id === row.id))
